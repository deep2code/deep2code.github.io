...
def remove_duplicate_navigation(content):
    """Remove page-nav and related-card sections (generated by generate_pages.py)."""
    # Remove all related-card sections (they are generated by generate_pages.py)
    while '<aside class="related-card">' in content:
        start = content.find('<aside class="related-card">')
        # Find the closing </aside>
        end = content.find('</aside>', start)
        if end == -1:
            break
        content = content[:start] + content[end + 9:]

    # Remove all page-nav sections (they are generated by generate_pages.py)
    while '<div class="page-nav">' in content:
        start = content.find('<div class="page-nav">')
...
def extract_page_content(html_text):
    ...
    # Find <main id="body-content"> (new format) or <div id="body-inner"> (old format)
    start = html_text.find('<main id="body-content">')
    if start == -1:
    ...
    # Find the end: </main> (new format) or <footer class="footline"> (old format)
    end_markers = [
        '</main>',  # new format
        '<footer class="footline">',  # old format
        '</div>\n\n</div>\n\n<div id="navigation">',
        '</div>\n        </div>\n        <div id="navigation">',
    ]
    end = len(html_text)
    ...
    if content.startswith('<main id="body-content">'):
        content = content.replace('<main id="body-content">', '', 1)
    elif content.startswith('<div id="body-inner">'):
    ...
    content = remove_duplicate_navigation(content)
